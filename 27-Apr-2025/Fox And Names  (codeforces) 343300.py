...
topo_order = []
if not impossible:
    UNVISITED, VISITED, VISITING = 0, 1, 2
    visit_state = [UNVISITED]*26
    def dfs(letter):
        if visit_state[letter] == VISITED:
            return False
        if visit_state[letter] == VISITING:
            return True
        visit_state[letter] = VISITING
        for neig in adj_list[letter]:
            is_cyc = dfs(neig)
            if is_cyc:
                return True
        topo_order.append(chr(letter + ord('a')))
        visit_state[letter] = VISITED
        return False

    for letter in range(26):
        if visit_state[letter] == UNVISITED:
            is_cycle = dfs(letter)
            if is_cycle:
                impossible = True
                break
...
if not impossible:
    print(''.join(reversed(topo_order)))
else:
    print('Impossible')